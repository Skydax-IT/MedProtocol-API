...
def _key_material(raw_key: str, pepper: str = "") -> bytes:
    if pepper:
        return f"{pepper}:{raw_key}".encode()
    return raw_key.encode()


def hash_api_key(raw_key: str, salt: str | None = None, pepper: str = "") -> str:
    salt_bytes = base64.urlsafe_b64decode(salt.encode("ascii")) if salt else secrets.token_bytes(16)
    digest = hashlib.pbkdf2_hmac(
        "sha256",
        _key_material(raw_key, pepper),
        salt_bytes,
        PBKDF2_ITERATIONS,
    )
...
def verify_api_key(raw_key: str, stored_hash: str, pepper: str = "") -> bool:
    try:
        ...
        return False
    actual = hashlib.pbkdf2_hmac("sha256", _key_material(raw_key, pepper), salt_bytes, iterations)
